    ...
    <div class="entry-preview relative flex flex-col w-full">
        ...
        <div class="h-full fill-neutral-500 stroke-neutral-500 shrink-0 flex items-center justify-center pointer-events-none">
            {#if entry.filename}
                ...
                {#if format === "image"}
                    <img use:loadFilePreview alt="" src="" data-src="/api/v1/file/image-thumbnail?size=256&path={encodeURIComponent(entry.path)}&modified={entry.modifiedDate}" class="h-full w-full object-contain opacity-0" on:load={(e) => { e.currentTarget.classList.remove("opacity-0") }}>
                {:else if format === "video"}
                    <img use:loadFilePreview alt="" src="" data-src="/api/v1/file/video-preview?size=256&path={encodeURIComponent(entry.path)}&modified={entry.modifiedDate}" class="h-full w-full object-contain opacity-0" on:load={(e) => { e.currentTarget.classList.remove("opacity-0") }}>
                {:else}
